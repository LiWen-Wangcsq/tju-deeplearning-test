...
class CSwinUnet3D(nn.Module):
    def __init__(self, *, hidden_dim, layers, heads, in_channel=1, num_classes=2, head_dim=32,
        ...
        self.cbam3 = CBAM(channel=hidden_dim)
    ...
    def forward(self, img):
        ...
        cbam1 = self.cbam1(up4)
        up3 = self.dec3(cbam1)  # ((B, 2C,X//8, Y//8, Z//8)
        # up2和 down2融合
        up3 = self.converge3(up3, down3)  # (B,2C, X//8, Y//8)

        cbam2 = self.cbam2(up3)
        up12 = self.dec12(cbam2)  # (B,C, X//4, Y//4, Z// 4)
        # up3和 down1融合
        up12 = self.converge12(up12, down12_1)  # (B,C, X//4, Y//4, Z//4)
        out = self.cbam3(up12)
        out = self.final(out)  # (B,num_classes, X, Y, Z)
        out = self.out(out)
        return out
